                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  int pos = numCols * absolute_image_position_y + absolute_image_position_x;
  ...
  float result = 0.f;
  int image_r, image_c;
  float image_value, filter_value;
  //For every value in the filter around the pixel (c, r)
  for (int filter_r = -filterWidth/2; filter_r <= filterWidth/2; ++filter_r) {
    for (int filter_c = -filterWidth/2; filter_c <= filterWidth/2; ++filter_c) {
      //Find the global image position for this filter position
      //clamp to boundary of the image
      image_r = min(max(absolute_image_position_y + filter_r, 0), static_cast<int>(numRows - 1));
      image_c = min(max(absolute_image_position_x + filter_c, 0), static_cast<int>(numCols - 1));
                       ...
                       int numCols)
{
  ...
  unsigned char red   = redChannel[pos];
  unsigned char green = greenChannel[pos];
  unsigned char blue  = blueChannel[pos];

  //Alpha should be 255 for no transparency
  uchar4 outputPixel = make_uchar4(red, green, blue, 255);

  outputImageRGBA[pos] = outputPixel;
}
                        ...
                        const int filterWidth)
{
  ...
  const dim3 gridSize(ceil(((float)numCols)/32), ceil(((float)numRows)/32), 1);
  ...
  // Now we recombine y our results. We take care of launching this kernel for you.
  //
